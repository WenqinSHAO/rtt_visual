...
def plotter(file, update):
    ...
    if not update and os.path.exists(out_html):
        return
...
def main():
    ...
    parser.add_argument('-u', '--update', action='store_true',
                        help='when present, update html output even already exist.')
    ...
    if args.directory:
        ...
        pool.map(plotter_wrapper, itertools.izip(target_file, itertools.repeat(args.update)))

    if args.file:
        if os.path.isfile(args.file):
            file = os.path.basename(args.file)
            if file.endswith('.csv') and not file.startswith('~'):
                plotter(args.file, args.update)
